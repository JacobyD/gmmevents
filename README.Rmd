...
We tested a number of Gaussian Mixture Model libraries for both Python and R programming language and VBmix library provided the most reliable results. The downside is that VBmix is currenly only available for Mac OS or Linux. However, this does mean that it can be run on cloud based platforms (e.g. Amazon Web Services, Azure (using linux), etc).

To run the analyses from a mapped directory, open 'run_code.R' and follow instructions.
